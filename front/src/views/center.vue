  ...
  <div>
    ...
    >  
     <el-row>
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='yonghu'"  label='房源' prop="fangyuanTypes">
                 <el-select v-model="ruleForm.fangyuanTypes"  placeholder='请选择房源'>
                     <el-option
                             v-for="(item,index) in fangyuanTypesOptions"
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='yonghu'"  label='户型' prop="huxingTypes">
                 <el-select v-model="ruleForm.huxingTypes"  placeholder='请选择户型'>
                     <el-option
...
// 注册表 用户 的级联表
   ...
   this.$http({
       url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=fangyuan_types`,
       method: "get"
   }).then(({ data }) => {
       if (data && data.code === 0) {
          this.fangyuanTypesOptions = data.data.list;
      } else {
          this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    onUpdateHandler() {
                         ...
                         if((!this.ruleForm.fangyuanTypes)&& 'yonghu'==this.flag){
                             this.$message.error('房源不能为空');
                             return
                         }

                         if((!this.ruleForm.huxingTypes)&& 'yonghu'==this.flag){
